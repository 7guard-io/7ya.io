...
export async function fetchFacebookPageBatch(ctx:MetaPageFetchContext):Promise<MetaBatch>{
  ...
  const params:Record<string,string>={
    fields:'id,message,created_time,permalink_url,full_picture,attachments{media_type,type,url,media,target}',
    limit:'50',
  };
  ...
  const records=(Array.isArray(payload.data)?payload.data:[]).flatMap(value=>{
    const record=normalizeFacebookPost(value,{apiVersion:ctx.config.apiVersion,accountObjectId:ctx.page.pageId,fetchedAt});
