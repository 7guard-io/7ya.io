...
const EVIDENCE_KINDS = new Set<EvidenceKind>(['fact', 'claim', 'statement', 'event', 'document', 'media', 'observation', 'inference', 'opinion', 'unknown']);
const VERIFICATION_LEVELS = new Set<VerificationLevel>(['primary-source', 'official-record', 'independent-source', 'self-report', 'derived', 'unverified']);
const VISIBILITIES = new Set<Visibility>(['public', 'private', 'restricted']);
...
export function createEvidenceAtom(input: CreateEvidenceAtomInput): EvidenceAtom {
  if (!EVIDENCE_KINDS.has(input.kind)) throw new TypeError('kind is invalid');
  if (!VISIBILITIES.has(input.visibility)) throw new TypeError('visibility is invalid');
  if (!VERIFICATION_LEVELS.has(input.verification.level)) throw new TypeError('verification.level is invalid');
