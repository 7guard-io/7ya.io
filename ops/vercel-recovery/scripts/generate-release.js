...
function resolveProviderProvenance() {
...
function resolveManualPreviewProvenance(environment) {
...
const provenance = resolveProviderProvenance() || resolveManualPreviewProvenance(environment);

if (!provenance) {
  throw new Error(
    'Release provenance is unbound. Production requires VERCEL_GIT_COMMIT_SHA or GITHUB_SHA. ' +
    'Manual preview binding additionally requires ALLOW_MANUAL_SOURCE_SHA=true.'
  );
}
...
const productionVerified =
  ...
  ['VERCEL_GIT_COMMIT_SHA', 'GITHUB_SHA'].includes(provenance.provenanceSource);
...
console.log(
  `⚡ ${manifest.release} bound to ${manifest.source_sha} via ${manifest.provenance_source} ` +
  `(${manifest.environment}, production_verified=${manifest.production_verified})`
);
