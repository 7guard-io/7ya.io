...
const meta=document.createElement('small');meta.textContent=[r.platform||'רשומה',r.year||'תאריך לא הושלם',r.political?'ארכיון אזרחי / פוליטי':r.object_type||'אלבום חיים'].join(' · ');el.append(meta);
...
fetch('/knowledge/master-public-record-20260918.json',{cache:'no-store'}).then(r=>r.json()).then(d=>{all=Array.isArray(d.records)?d.records:[];render()}).catch(()=>{grid.innerHTML='<p>מאגר החיים המלא לא נטען כרגע.</p>'});
})();