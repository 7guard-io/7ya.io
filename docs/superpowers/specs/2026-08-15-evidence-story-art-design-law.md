...
7YA must not present Igor Vepretski's archive as a raw data dump, a social feed, or a generic portfolio. Every public-facing historical or influence asset must be selected and transformed according to one governing rule:

> **Evidence proves it. Story gives it meaning. Design makes you feel it. Action makes it yours.**

The public site uses **Evidence × Story × Art** as the mandatory composition model.

- **Evidence** establishes that the event, publication, reaction, relationship, metric or outcome is real.
- **Story** explains why the item matters inside Igor's life and public trajectory.
...
Each candidate asset receives an internal editorial score:
...
A complete scene contains, when available:

1. **Dominant visual** — original photograph, video still, document, post capture, press image or generated metaphor clearly labelled as illustrative.
2. **Human statement** — one concise sentence explaining what happened.
3. **Tension** — the problem, loss, contradiction, risk or question present at that moment.
4. **Action** — what Igor actually did.
5. **Impact signal** — reaction, redistribution, discussion, declared action, institutional response or documented outcome.
6. **Evidence bridge** — a visible or expandable source path with evidence tier and date.
7. **Visitor mirror** — a short reflective prompt connecting the scene to the visitor's own life.
8. **Next move** — route into the relevant 7YA room, companion conversation, creation flow, StartOn layer or deeper evidence.
...
For every important interview, TV appearance, video podcast, public lecture or stage appearance, the system should identify one or more **Hero Frames**:

- select a frame with strong expression, posture, eye line and contextual readability;
- prefer frames where Igor is clearly recognizable and visually dominant;
- repair crop and perspective when needed;
...
`EXPOSURE → RESONANCE → PROPAGATION → TRANSFORMATION`

Examples:

- views establish exposure;
- likes, saves, substantive comments and watch behavior establish resonance at different strengths;
- shares, reposts, quotes, syndication and cross-platform pickup establish propagation;
- declared vote intention, joining a project, creating derivative content, institutional action or other attributable behavioral change establish transformation.
...
A **Ghost Profile / Removed Surface** scene may include:

- original and later handles;
- active period;
- verified historic metrics;
- surviving external references;
- content families that originated there;
- downstream reposts, media pickup or mirrors;
- reason for disappearance only when verified or clearly labelled as owner-reported;
...
Private material may contribute to understanding, chronology, verification and editorial choice, but it must not flow directly to the public site.

Pipeline:

`PRIVATE SOURCE → NORMALIZED RECORD → CLAIM REVIEW → PUBLIC-SAFE EVIDENCE → STORY SCENE`

Never publish private correspondence, phone lists, minors' information, credentials, legal/financial records, private family data, access tokens or private Drive URLs.
...
- crop repair;
- reframing for responsive layouts;
- exposure and tonal correction;
- color correction;
- perspective correction;
- denoise;
- restrained sharpening;
- background cleanup only when it does not alter historical meaning;
- legibility restoration;
- subtitle/transcript overlays clearly separated from the source image.

Not allowed for evidence imagery:

- changing identity or facial structure;
- fabricating expressions;
- adding people who were not present;
- removing participants whose presence changes context;
...
`MEET IGOR → LIVE THE STORY → SEE THE EVIDENCE → SEE THE RESPONSE → UNDERSTAND THE TRANSFORMATION → RECOGNIZE YOURSELF → ACT`
...
- it exists only because the metric is large;
- it lacks a meaningful connection to the larger life map;
- its core claim has no source or evidence state;
- a generated image could be mistaken for authentic documentation;
...
The strongest scenes should achieve all four outcomes:

1. **I believe this happened.**
2. **I understand why it mattered.**
3. **I felt the person behind it.**
4. **It made me think about what I can do next.**

That is the 7YA standard.
