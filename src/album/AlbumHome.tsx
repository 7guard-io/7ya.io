...
import RichLifeTimeline from './RichLifeTimeline';
import FacebookStoryStream from '../FacebookStoryStream';
import OwnerFacebookArchive from './OwnerFacebookArchive';
import LifeThroughline from '../LifeThroughline';
import LifeMediaStrip from './LifeMediaStrip';
import LazyYouTube from '../LazyYouTube';
import './album.css';
import './album-story-traces.css';


const voiceCopy: Record<Locale, {eyebrow: string; title: string; body: string; firstTitle: string; secondTitle: string; source: string; more: string}> = {
    he: {
        eyebrow: 'FIRST PERSON · ORIGINAL VOICE',
        title: 'לא תקציר עליי. תנו לי לדבר.',
        body: 'לפני הציר והכותרות — שתי כניסות קצרות לשפה, להומור ולזהות שמהם יצאתי. אלה סרטונים מקוריים מהערוץ שלי, לא קריינות ולא סיכום של מישהו אחר.',
        firstTitle: 'חינוך רוסי. אחד השורשים הוויראליים שלי.',
        secondTitle: 'אבא רוסי',
        source: 'לצפייה ב־YouTube',
        more: 'לעוד וידאוים וסיפורים',
    },
    en: {
        eyebrow: 'FIRST PERSON · ORIGINAL VOICE',
        title: 'Not a summary of me. Let me speak.',
        body: 'Before the timeline and the headlines: two short entries into the language, humour and identity I came from. These are original videos from my channel — not narration and not somebody else’s summary.',
        firstTitle: 'Russian education. One of my viral roots.',
        secondTitle: 'Russian father',
        source: 'Watch on YouTube',
        more: 'More videos and stories',
    },
    ru: {
        eyebrow: 'FIRST PERSON · ORIGINAL VOICE',
        title: 'Не пересказ обо мне. Дайте мне говорить.',
        body: 'До хронологии и заголовков — два коротких входа в язык, юмор и идентичность, из которых я вышел. Это оригинальные видео с моего канала, не озвучка и не чужой пересказ.',
        firstTitle: 'Русское воспитание. Один из моих вирусных корней.',
        secondTitle: 'Русский папа',
        source: 'Смотреть на YouTube',
        more: 'Больше видео и историй',
    },
};

function IgorVoiceEntry({locale}: {locale: Locale}) {
    const c = voiceCopy[locale];
    const videos = [
        {id: '5qxA4hgUhV8', title: c.firstTitle, href: 'https://www.youtube.com/watch?v=5qxA4hgUhV8', eager: true},
        {id: 'xQBFuu5qE6E', title: c.secondTitle, href: 'https://www.youtube.com/shorts/xQBFuu5qE6E', eager: false},
    ];

    return (
        <section className='igor-voice-entry' id='igor-voice' aria-labelledby='igor-voice-title'>
            <header>
                <p dir='ltr'>{c.eyebrow}</p>
                <h2 id='igor-voice-title'>{c.title}</h2>
                <p>{c.body}</p>
            </header>
            <div className='igor-voice-grid'>
                {videos.map(video => (
                    <article className='igor-voice-card' key={video.id}>
                        <LazyYouTube
                            videoId={video.id}
                            title={video.title}
                        ...
                        />
                        <footer>
                            <small dir='ltr'>IGOR VEPRETSKI · ORIGINAL PUBLIC VIDEO</small>
                            <a href={video.href} target='_blank' rel='noreferrer'>{c.source}<ExternalLink /></a>
                        </footer>
                    </article>
                ))}
            </div>
            <a className='igor-voice-more' href={pageHref('media', locale)}>{c.more}<ArrowUpRight /></a>
        </section>
    );
}
...
const copy = {
    he: {
        ...
        enter: 'להכיר אותי בקול',
        talk: 'לדבר / לשתף פעולה',
        opening: 'החיים, בזמן שהם קרו.',
        openingBody: 'זה כבר לא מסלול שסוגר חיים בתוך מספר קבוע של פרקים. ה־Life Atlas מחבר את הקאנון, הפרסומים החיים, ארכיון legacy ושכבת Discovery לציר כרונולוגי שיכול להמשיך לגדול בכל פעם שמקור אמיתי נוסף נכנס.',
        age: 'גיל',
        place: 'מקום',
        people: 'אנשים',
        moment: 'הרגע',
    ...
    en: {
        ...
        openingBody: 'This is no longer a life path capped by a fixed number of chapters. The Life Atlas connects Canon, live publications, the legacy archive and Discovery into a chronology that can keep growing whenever another real source enters the record.',
        age: 'AGE',
        place: 'PLACE',
        people: 'PEOPLE',
        moment: 'THE MOMENT',
    ...
    ru: {
        ...
        enter: 'Сначала послушать меня',
        talk: 'Поговорить / сотрудничать',
        opening: 'ЖИЗНЬ, ПОКА ОНА ПРОИСХОДИЛА',
        openingBody: 'Это больше не жизненный путь, ограниченный фиксированным числом глав. Life Atlas соединяет Canon, живые публикации, legacy-архив и Discovery в хронологию, которая растёт вместе с появлением новых реальных источников.',
        age: 'ВОЗРАСТ',
        place: 'МЕСТО',
        people: 'ЛЮДИ',
        moment: 'МОМЕНТ',
...
const chapterTraceIds: Record<AlbumChapter['id'], readonly string[]> = {
    origin: ['early-2011', 'school-note-reflection-2022', 'childhood-gilat-starton-2023', 'ndi-repatriation-2024', 'mindset-page'],
    service: ['early-2011', 'mfa-miami-consular-2012', 'police-exit-2023', 'police-uniform-fatherhood-2024', 'police-reel'],
    ...
    if (broken) {
        return (
            <div className='album-cover-fallback'>
                ...
                <a href={coverMedia.sourceUrl} target='_blank' rel='noreferrer'>{coverMedia.sourceLabel} ↗</a>
            </div>
        );
    }
    return (
...
export default function AlbumHome() {
    ...
    useEffect(() => {
        ...
        if (!hostname.endsWith('.appdeploy.ai') || params.has('visualAcceptance') || params.has('visualGateCapture')) return;
        const viewport = window.matchMedia('(max-width: 640px)').matches ? 'mobile' : 'desktop';
        void api.get('/api/visual-acceptance?path=home&viewport=' + viewport).catch(() => undefined);
    }, []);
    ...
    return (
        <main className='album-home' dir={dir}>
            <section className='album-cover' id='album-cover'>
                <div className='album-cover-copy'>
                    ...
                    <div className='album-actions'>
                        <a className='album-action-primary' href={isDeepAlbum ? '#album-journey' : '#igor-voice'}>{c.enter}<ArrowDown /></a>
            ...
            <IgorVoiceEntry locale={locale} />

            <LifeThroughline />

            {!isDeepAlbum && <LifeMediaStrip />}

            {isDeepAlbum && <OwnerFacebookArchive />}

            <FacebookStoryStream mode={isDeepAlbum ? 'media' : 'home'} />
            ...
            {isDeepAlbum && (
                <>
                    <section className='album-opening' id='album-journey'>
                        ...
                        <RichLifeTimeline context='album' />
                    </section>

                    <section className='album-anchor-intro' aria-labelledby='album-anchor-title'>
                        <small dir='ltr'>CURATED ANCHOR STORIES / DEPTH · NOT THE FULL LIFE</small>
                        <h2 id='album-anchor-title'>{locale === 'he' ? 'סיפורי עוגן שנבחרו לעומק — לא גבול למסע.' : locale === 'ru' ? 'Опорные истории для глубины — не граница жизненного пути.' : 'Curated anchor stories for depth — not a ceiling on the life.'}</h2>
                        <p>{locale === 'he' ? 'הפרקים הערוכים נשארים כי הם נותנים הקשר, רפלקציה והמשך. אבל הם כבר שכבת עומק מתחת לציר החי, ולא הרשימה שמחליטה כמה מהחיים נכנס לאתר.' : locale === 'ru' ? 'Отредактированные главы остаются как слой контекста, рефлексии и продолжения. Теперь они находятся под живой хронологией и не решают, сколько жизни помещается на сайте.' : 'The edited chapters remain because they add context, reflection and consequence. They now sit beneath the living chronology and no longer decide how much of the life fits on the site.'}</p>
                    </section>

                    <section className='album-chapters' aria-label='Curated personal album anchor stories'>
            ...
            {isDeepAlbum ? <PostsMemoryUniverse /> : <PostsMemoryUniverse mode='portal' />}
            {isDeepAlbum ? <BroadcastStream /> : <BroadcastStream mode='portal' />}
