...
Verified AppDeploy source snapshot: `1787553133110`
Latest QA screenshot snapshot: `1787553182279`

## Purpose

This release moves 7YA from a scene-aware conversational assistant toward a generative interface while preserving evidence boundaries. Digital Igor can construct a Story Path from the public canon, use that path to control real documentary scenes, and keep the generated route available as site navigation after the chat closes. In parallel, the public archive and Digital History no longer depend on asynchronous enrichment in order to show that a verified life corpus exists.

## Generative Story Paths

A reusable public primitive exists at `GET /api/story-path`.
...
The Story Path chapter sequence uses canonical life chronology for path semantics. The StartOn path deliberately uses `starton-return-2022`, not the separate nonprofit-registration record, because the mapped visual scene is the return to Jesse Cohen / StartOn story.

## Digital Igor interface

`POST /api/companion` returns an optional `storyPath` in addition to reply, evidence, actions and checkpoint.
...
### Persistent Story Path HUD

The generated path is also navigation state, not only chat-response state:

- a compact `STORY PATH · CANON ONLY` HUD remains available after the chat closes;
- the HUD exposes every generated canonical node as a direct scene control;
- the currently visible mapped scene is marked as the active step;
- the path is stored in session-scoped browser storage and therefore survives route changes without becoming permanent profile data;
- the visitor can dismiss the path explicitly;
- resetting Digital Igor also clears the path.

This is the first production step toward a generative interface in which conversation changes how the site is traversed rather than merely returning text.

## Canonical-first boot

A public search crawl had surfaced a stale/first-render representation in which `100 MOMENTS` appeared as zero and Digital History showed an unavailable state. The active application source already had a real canonical corpus, so the correct fix was not to invent fallback metrics or content.
...
- enrichment failure leaves a canonical chronology visible rather than replacing it with an empty/unavailable story.
...
Story Path persistence uses `sessionStorage`; it does not create a server-side personal profile or private memory.

## Verification

Fresh source verification on AppDeploy snapshot `1787553133110` confirmed together:
...
- visible `STORY PATH · CANON ONLY` chat rail;
- session-backed `storyPathStorageKey` and persistent Story Path HUD;
...
Terminal AppDeploy deployment state was `ready` with zero frontend, backend and network errors. Fresh desktop and mobile QA screenshots were generated in snapshot `1787553182279`.
