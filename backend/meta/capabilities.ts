...
export async function discoverMetaCapabilities(config:MetaConfig):Promise<MetaCapabilityDiscovery>{
  ...
  try{
    ...
    const report:MetaCapabilityReport={checkedAt,apiVersion:config.apiVersion,pages,discoveredPageCount:pages.length,allowedPageCount:pages.filter(page=>page.allowed).length,linkedInstagramCount:pages.filter(page=>Boolean(page.instagram)).length,allowedInstagramCount:pages.filter(page=>page.instagram?.allowed).length,state:'ready',grantedPermissions};
