...
const proofItems:ProofItem[]=[
  ...
  },
  {
    ...
    image:'https://i.ytimg.com/vi/3XxoBtSL2pg/hqdefault.jpg',
  ...
  },
  {
    ...
    image:'https://i.ytimg.com/vi/O3v309CA4ao/hqdefault.jpg',
  ...
  },
  {
    ...
    image:'https://i.ytimg.com/vi/SOpAglwkJ8I/hqdefault.jpg',
...
function ProofVisual({item,index,locale}:{item:ProofItem;index:number;locale:Locale}){
  const [src,setSrc]=useState(item.image);
  const [failed,setFailed]=useState(false);
  const fail=()=>{
    if(item.fallback&&src!==item.fallback){setSrc(item.fallback);return}
    setFailed(true);
  };
  return <figure className={failed?'pm-visual-failed':''}>
    <div className='pm-source-poster'><small>{item.kind==='owner'?'OWNER ARCHIVE':'SOURCE MEDIA'}</small><strong>{item.publisher}</strong><span>{item.year}</span></div>
    {!failed&&<img src={src} alt={item.title[locale]} loading={index<3?'eager':'lazy'} decoding='async' referrerPolicy='no-referrer' onError={fail}/>} 
    {item.kind==='broadcast'&&<i aria-hidden='true'><Play fill='currentColor'/></i>}
    <em>{item.kind==='owner'?sectionCopy[locale].owned:sectionCopy[locale].media}</em>
  </figure>
}
