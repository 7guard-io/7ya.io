...
const SOURCE_SHA = '7263e187ed468394d7866412ee365fcbb3d4105b';
...
function resolveSourcePath(pathInfo) {
  ...
  return extension(pathInfo.normalized) ? pathInfo.normalized : `${pathInfo.normalized}/index.html`;
}
...
async function fetchSource(file) {
  return fetch(`${RAW_BASE}${file}`, {
    headers: {
      'User-Agent': '7ya-canonical-personal-release/2.0',
      Accept: '*/*',
    },
  });
}
...
module.exports = async (request, response) => {
  ...
  const pathInfo = parseRequestPath(request);
  if (!pathInfo) {
  ...
  if (pathInfo.normalized && BLOCKED_PUBLIC_PREFIXES.has(pathInfo.normalized.split('/')[0])) {
    sendNotFound(request, response);
    return;
  }

  const aliasKey = pathInfo.normalized.replace(/\/+$/, '');
  const alias = CANONICAL_ALIASES.get(aliasKey);
  if (alias) {
  ...
  try {
    ...
    console.error('7YA canonical personal proxy failure', error?.message || error);
